...
with the freedomcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a freedomcoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Freedomcoin Core node application logic. For custom behaviour, subclass the
...
Test objects for interacting with a freedomcoind node over the p2p interface.
