...
#include <arith_uint256.h> // Include this for arith_uint256 and related functions
...
static CBlock CreateGenesisBlock(uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward) {
    const char* pszTimestamp = "FreedomCoin Genesis Block";
...
public:
    CMainParams() {
        ...
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nStartHeight = 210240; // Approx. one year after launch
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nTimeoutHeight = 420480; // Approx. two years after launch

        // Deployment of MWEB (LIP-0002, LIP-0003, and LIP-0004)
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].bit = 4;
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].nStartHeight = 210240; // Approx. one year after launch
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].nTimeoutHeight = 420480; // Approx. two years after launch

        // Update timestamp for MWEB (LIP-0002, LIP-0003, and LIP-0004)
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].nStartTime = 1723248000; // August 10, 2024
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].nTimeout = 1786128000; // August 10, 2026 (Approx. two years after start)
      ...
      //  std::cout << "MainNet Genesis Block: " << consensus.hashGenesisBlock.ToString() << std::endl;
       // std::cout << "MainNet Merkle Root: " << genesis.hashMerkleRoot.ToString() << std::endl;
        ...
        vSeeds.emplace_back("seed.freedomcoin.org");

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,50); // Starts with 'F'
...
public:
    CTestNetParams() {
        ...
        // Deployment of Taproot (BIPs 340-342)
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].bit = 2;
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nStartHeight = 2225664; // Set to a future height
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nTimeoutHeight = 2435328; // 364 days later

        // Deployment of MWEB (LIP-0002, LIP-0003, and LIP-0004)
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].bit = 4;
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].nStartHeight = 2230000; // Set to a future height
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].nTimeoutHeight = 2430000; // 364 days later
       ...
       // std::cout << "TestNet Merkle Root: " << genesis.hashMerkleRoot.ToString() << std::endl;
        ...
        vSeeds.emplace_back("testnet-seed.freedomcoin.org");
...
void CRegTestParams::UpdateActivationParametersFromArgs(const ArgsManager& args)
{
    ...
    for (const std::string& strDeployment : args.GetArgs("-vbparams")) {
        ...
        if (vDeploymentParams.size() > 3 && !ParseInt64(vDeploymentParams[3].c_str(), &nStartHeight)) {
            throw std::runtime_error(strprintf("Invalid nStartHeight (%s)", vDeploymentParams[3]));
        }
        if (vDeploymentParams.size() > 4 && !ParseInt64(vDeploymentParams[4].c_str(), &nTimeoutHeight)) {
