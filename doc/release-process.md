...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/freedomcoin-project/gitian.sigs.ltc.git
    git clone https://github.com/freedomcoin-project/freedomcoin-detached-sigs.git
    git clone https://github.com/devrandom/gitian-builder.git
    git clone https://github.com/freedomcoin-project/freedomcoin.git
...
### Build and sign Freedomcoin Core for Linux, Windows, and macOS:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit freedomcoin=v${VERSION} ../freedomcoin/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-linux --destination ../gitian.sigs.ltc/ ../freedomcoin/contrib/gitian-descriptors/gitian-linux.yml
    mv build/out/freedomcoin-*.tar.gz build/out/src/freedomcoin-*.tar.gz ../

    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit freedomcoin=v${VERSION} ../freedomcoin/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-win-unsigned --destination ../gitian.sigs.ltc/ ../freedomcoin/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit freedomcoin=v${VERSION} ../freedomcoin/contrib/gitian-descriptors/gitian-osx.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-unsigned --destination ../gitian.sigs.ltc/ ../freedomcoin/contrib/gitian-descriptors/gitian-osx.yml
...
Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../freedomcoin/contrib/gitian-keys/README.md`.

Verify the signatures

    pushd ./gitian-builder
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-linux ../freedomcoin/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-win-unsigned ../freedomcoin/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-osx-unsigned ../freedomcoin/contrib/gitian-descriptors/gitian-osx.yml
    popd
...
Codesigner only: Sign the macOS binary:
    ...
    $   xcrun altool --notarize-app --primary-bundle-id "org.freedomcoin.Freedomcoin-Qt" -u "<apple-id-email>" -p "@keychain:<apple-id-notarisation-app-specific-password>" --asc-provider <team-id-shortcode> -t osx -f freedomcoin-${VERSION}-osx.dmg
    ...
    $   xcrun stapler staple dist/Freedomcoin-Qt.app
...
Codesigner only: Commit the detached codesign payloads:
    ...
    cp dist/Freedomcoin-Qt.app/Contents/CodeResources osx/dist/Freedomcoin-Qt.app/Contents/
    git add -A
...
- Detached signatures will then be committed to the [freedomcoin-detached-sigs](https://github.com/freedomcoin-project/freedomcoin-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.

Create (and optionally verify) the signed macOS binary:

    pushd ./gitian-builder
    ./bin/gbuild -i --commit signature=v${VERSION} ../freedomcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-signed --destination ../gitian.sigs.ltc/ ../freedomcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gverify -v -d ../gitian.sigs.ltc/ -r ${VERSION}-osx-signed ../freedomcoin/contrib/gitian-descriptors/gitian-osx-signer.yml
...
space *do not upload these to the freedomcoin.org server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the freedomcoin.org server.

- Update freedomcoin.org version
...
- Announce the release:
  ...
  - freedomcoin-dev mailing list

  - blog.freedomcoin.org blog post

  - Update title of #freedomcoin and #freedomcoin-dev on Freenode IRC

  - Optionally twitter, reddit /r/Freedomcoin, ... but this will usually sort out itself

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/freedomcoin-project/freedomcoin/releases/new) with a link to the archived release notes.
