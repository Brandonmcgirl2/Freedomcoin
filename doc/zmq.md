...
The Freedomcoin Core daemon can be configured to act as a trusted "border
router", implementing the freedomcoin wire protocol and relay, making
...
The ZeroMQ feature in Freedomcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building freedomcoind:
...
For instance:
    ...
    $ freedomcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/freedomcoind.tx.raw \
...
These options can also be provided in freedomcoin.conf.
...
From the perspective of freedomcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into freedomcoind directly. Furthermore, no information is
...
using. Freedomcoind appends an up-counting sequence number to each
