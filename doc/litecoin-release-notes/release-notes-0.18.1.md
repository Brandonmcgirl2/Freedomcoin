Freedomcoin Core version 0.18.1 is now available from:

  <https://download.freedomcoin.org/freedomcoin-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/freedomcoin-dev>
...
`/Applications/Freedomcoin-Qt` (on Mac) or `freedomcoind`/`freedomcoin-qt` (on
Linux).
...
Freedomcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Freedomcoin Core on unsupported systems.
...
10.10. Additionally, Freedomcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@freedomcoin.org) the Freedomcoin Core
...
- A new short [document](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/freedomcoin-conf.md)
  about the `freedomcoin.conf` file describes how to use it to configure
  Freedomcoin Core.

- A new document introduces Freedomcoin Core's BIP174 [Partially-Signed
  Freedomcoin Transactions
  (PSBT)](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Freedomcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Freedomcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/freedomcoin-project/freedomcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Freedomcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Freedomcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/freedomcoin-project/freedomcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart freedomcoind with the `-deprecatedrpc=generate`
...
- A new `freedomcoin-wallet` tool is now distributed alongside Freedomcoin
...
This section describes planned changes to Freedomcoin Core that may affect
other Freedomcoin software and services.

- Since version 0.16.0, Freedomcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Freedomcoin Core 0.20 (expected about
  a year after 0.18), Freedomcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Freedomcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Freedomcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Freedomcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Freedomcoin Core
...
- A sub-project of Freedomcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Freedomcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Freedomcoin Core's own implementation, although entropy
  gathered by Freedomcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Freedomcoin Core a
...
- On macOS, Freedomcoin Core now opts out of application CPU throttling
